...
search.addEventListener("click", () => {
  let recipeContainers = document.querySelectorAll(".recipe");
  recipeContainers.forEach(container => {
    container.remove();
  })

  let key = searchFor.value.toLowerCase();
  const foods = recipes.filter(recipe => {
    return recipe.name.toLowerCase().includes(key);
  })
  ...
  foods.forEach(food => {
    ...
    <div class="recipe">
        ...
        <div class="desc">
            ...
            <span class="rating-${index}" role="img">
            </span>
            <p id="desc">${food.description}</p>
        </div>
    </div>`);

    const tagsContainer = document.querySelector(`.recipe-tags-${index}`);
    food.tags.forEach(tag => {
      tagsContainer.innerHTML += `<button>${tag}</button>`;
    })

    const rating = document.querySelector(`.rating-${index}`);
    rating.ariaLabel = `${food.rating} stars out of 5`;
    
    for (let i = 0; i < 5; i++) {
      if (i >= food.rating) {
        rating.innerHTML += `<span aria-hidden="true" class="icon-star-empty">☆</span>`;
      } else {
        rating.innerHTML += `<span aria-hidden="true" class="icon-star">⭐</span>`;
      }
    }
    index += 1;
  })
