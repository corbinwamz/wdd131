...
search.addEventListener("click", () => {
  ...
  const trails = hikes.filter(hike => {
    return hike.name.toLowerCase().includes(key);
  })
  let index = 1;
  trails.forEach(trail => {
    ...
    const tagsContainer = document.querySelector(`.hike-tags-${index}`);
    trail.tags.forEach(tag => {
      tagsContainer.innerHTML += `<button>${tag}</button>`;
    })
    const difficulty = document.querySelector(`.rating-${index}`);
    difficulty.ariaLabel = `${trail.difficulty} out of 5 difficulty`;
    for (let i = 0; i < 5; i++) {
      if (i >= trail.difficulty) {
        difficulty.innerHTML += `▫️`;
      } else {
